...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build the query - only join with subscription_plans, not users
    ...
    if (plan && plan !== 'all') {
      // Get plan IDs that match the plan name
      ...
      if (planData && planData.length > 0) {
        query = query.in('plan_id', planData.map(p => p.id));
      }
    }
    ...
    // Enhance subscription data with user info, payments, and usage
    const enhancedSubscriptions = await Promise.all(
      subscriptions.map(async (sub) => {
        try {
          ...
          // Apply search filter here (on user data)
          if (search) {
          ...
          const lastPaymentDate = payments?.find(p => 
    ...
    const filteredSubscriptions = enhancedSubscriptions.filter(sub => sub !== null);
